...
{
    const CUSTOM_BLOCK_TEMPLATE = 'st_menu_idcat_%d';
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false,$item)
    {
        ...
        $id = $category->getId();
        // --- Static Block ---
        $blockId = sprintf('st_menu_idcat_%d', $id); // --- static block key
        $blockHtml = Mage::app()->getLayout()->createBlock('cms/block')->setBlockId($blockId)->toHtml();
        /*check block right*/
        $blockIdRight = sprintf('st_menu_idcat_%d_right', $id); // --- static block key
        ...
        if ($drawPopup)
        {
            $html[] = '<div id="st_menu' . $id . '" class="st_menu' . $active . ' nav-' .$item. '">';
        }
        else
        {
            $html[] = '<div id="st_menu' . $id . '" class="st_menu' . $active . ' nav-' .$item. ' st_menu_no_child">';
        }
        // --- Top Menu Item ---
...
//        print_r($arr_catsid);die();
    ...
    public function drawCustomMenuBlock($blockId)
    {
        ...
        $html = array();
        // --- Static Block ---
        //$blockId = sprintf('st_custommenu_%d', $id); // --- static block key
        ...
        if ($drawPopup)
        {
            $html[] = '<div id="st_menu' . $id . '" class="st_menu">';
        }
        else
        {
            $html[] = '<div id="st_menu' . $id . '" class="st_menu">';
